<main>
  <div class="container bg-primary text-black text-center">
    <h1 class="mb-3">Résultats de covoiturages</h1>
    <!-- Barre de filtres -->
    <div class="container mt-4">
      <!-- Bouton Hamburger (visible seulement sur petit écran) -->
      <button
        class="btn btn-outline-secondary filter-toggle bg-dark align-self-lg-center"
        type="button"
        data-bs-toggle="collapse"
        data-bs-target="#filtreCollapse">
        ☰ Filtres
      </button>

      <!-- Barre de filtres (collapsible sur petit écran) -->
      <div class="collapse d-md-block" id="filtreCollapse">
        <div
          class="filter-bar p-3 bg-secondary rounded shadow-sm mb-4 d-flex justify-content-center">
          <form
            action="/covoiturage"
            method="get"
            class="row gy-3 gx-4 justify-content-center align-items-center w-100"
            style="max-width: 1200px">
            <!-- Catégorie -->
            <!--Type de trajet-->
            <div class="col-12 col-md-6 col-lg-3">
              <label
                for="typeTrajet"
                class="form-label bg- rgba(217, 217, 217, 0.5)">type de trajet</label>
              <select class="form-select" id="typeTrajet" name="typeTrajet">
                <option>Toutes</option>
                <option>Ecologique</option>
                <option>Standard</option>
              </select>
            </div>
            <!--Prix max-->
            <div class="col-12 col-md-6 col-lg-3">
              <label for="prixMax" class="form-label">Prix maximum (Crédits)</label>
              <input
                type="number"
                class="form-control"
                id="prixMax"
                name="prixMax"
                placeholder="Ex : 20"
                min="0" />
            </div>
            <!--Durée max-->
            <div class="col-12 col-md-6 col-lg-3">
              <label for="dureeMax" class="form-label">Durée maximum (Heures)</label>
              <input
                type="number"
                class="form-control"
                id="dureeMax"
                name="dureeMax"
                placeholder="Ex : 1"
                min="0" />
            </div>

            <!--Note Max-->
            <div class="col-12 col-md-6 col-lg-3">
              <label for="noteMin" class="form-label">Note minimale</label>
              <input
                type="number"
                class="form-control"
                id="noteMin"
                name="noteMin"
                max="10"
                min="0" />
            </div>
          </form>
        </div>
      </div>
      <!--Bouton de soumission-->
      <div class="col-12 text-center">
        <button type="submit" class="btn btn-dark">
          Appliquer les filtres
        </button>
      </div>

      <!-- Zone d'affichage des résultats -->
      <div id="covoiturages-results">
        <?php if (!empty($covoiturages)): ?>
          <?php foreach ($covoiturages as $trajet): ?>
            <div class="card mb-3 p-3 bg-light">
              <h5><?= htmlspecialchars($trajet['conducteur_pseudo']) ?></h5>
              <p>
                Note :
                <?= htmlspecialchars($trajet['conducteur_note']) ?>
                ★
              </p>
              <p>
                Départ :
                <?= htmlspecialchars($trajet['depart']) ?>
              </p>
              <p>
                Arrivée :
                <?= htmlspecialchars($trajet['arrivee']) ?>
              </p>
              <p>
                Date :
                <?= date('d/m/Y H:i', strtotime($trajet['date_depart'])) ?>
              </p>
              <p>
                Prix :
                <?= htmlspecialchars($trajet['prix']) ?>
                €
              </p>
              <p>
                Places disponibles :
                <?= htmlspecialchars($trajet['places_disponibles']) ?>
              </p>
              <p>
                Véhicule :
                <?= htmlspecialchars($trajet['vehicule_marque']) ?>
                <?= htmlspecialchars($trajet['vehicule_modele']) ?>
                (<?= htmlspecialchars($trajet['vehicule_energie']) ?>)
              </p>
              <p>
                Type de trajet :
                <?= $trajet['est_ecologique'] ? 'Écologique' : 'Standard' ?>
              </p>
              <?php if (isset($_SESSION['user_id'])): ?>
                <?php if (
                  $trajet['places_disponibles'] >
                  0
                ): ?>
                  <button
                    class="btn btn-success reserver-btn"
                    data-trajet-id="<?= $trajet['id'] ?>">
                    Réserver
                  </button>
                <?php else: ?>
                  <button class="btn btn-secondary" disabled>Complet</button>
                <?php endif; ?>
              <?php else: ?>
                <a href="/login" class="btn btn-info">Connectez-vous pour réserver</a>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <p>Aucun covoiturage trouvé avec ces critères.</p>
        <?php endif; ?>
      </div>
    </div>
  </div>
</main>